...
void FileMetaData::setPath(const QString &newPath)
{
    ...
    const QMimeType mime = m_db.mimeTypeForFile(newPath);
    ...
    if (
        s_archive_mimetypes.contains(mime.name()) ||
...
QString FileMetaData::md5Hash()
{
    if (m_md5Hash == "--") {
        auto *md5 = new HashCalculator(m_path, QCryptographicHash::Md5, nullptr);
        connect(md5, &HashCalculator::resultReady, this, [&](const QString &result){
            setMd5Hash(result);
        });
        connect(md5, &HashCalculator::finished, md5, &QObject::deleteLater);
        connect(this, &FileMetaData::destroyed, md5, &QObject::deleteLater, Qt::QueuedConnection);
        md5->start();
    }
    return m_md5Hash;
}
...
QString FileMetaData::sha256Hash()
{
    if (m_sha256Hash == "--") {
        auto *sha256 = new HashCalculator(m_path, QCryptographicHash::Sha256, nullptr);
        connect(sha256, &HashCalculator::resultReady, this, [&](const QString &result){
            setSha256Hash(result);
        });
        connect(sha256, &HashCalculator::finished, sha256, &QObject::deleteLater);
        connect(this, &FileMetaData::destroyed, sha256, &QObject::deleteLater, Qt::QueuedConnection);
        sha256->start();
    }
    return m_sha256Hash;
}
